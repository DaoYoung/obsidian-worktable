...
export function mountLearningWidget(containerEl: HTMLElement, context: WidgetContext): void {
  ...
  async function organizeConcept(regenerate: boolean): Promise<void> {
    ...
    try {
      const markdown = await client.expandKnowledge(name, conceptContext.value.trim());
      if (!markdown.trim()) throw new Error("AI 返回为空");
      if (disposed) return;
      pendingMarkdown = markdown;
      pendingName = name;
      previewBody.empty();
      await MarkdownRenderer.render(app, markdown, previewBody, knowledgePath, component);
  ...
  async function confirmConceptWrite(): Promise<void> {
    if (!pendingMarkdown || !pendingName) {
      conceptStatus.className = "kp-status err";
    ...
    try {
      const category = knowledge.inferCategory(pendingName, pendingMarkdown);
      await knowledge.append(category, pendingName, pendingMarkdown);
      localStorage.removeItem(KNOWLEDGE_CACHE_KEY);
      if (disposed) return;
      conceptStatus.className = "kp-status ok";
      conceptStatus.empty();
      conceptStatus.appendText("已写入 ");
